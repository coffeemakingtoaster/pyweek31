...
from .helper\Section import Section
...
class Guard(Actor.Actor):
    ...
    def raycast(self, degree, length, walls):
        ...
        for wall in walls:
            ...
            intersection = Point((-ray.startPoint[1]+wall.startPoint[1])/(m-m_wall),(ray.startPoint[1]+m*pointX))
            distance = distance(ray.startPoint,ray.endPoint)
            if distance > length:
                intersection = Point(999,999)
            if !(wall.startPoint.x < intersection.x) and (wall.endPoint > intersection.x):
                intersection = Point(999,999)
